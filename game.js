(() => {
  ...
  let path=[],totalLength=0,walls=[],pegs=[],funnels=[],bounds={minX:0,minY:0,maxX:1,maxY:1};
  ...
  function updateCamera(){
    const lead=currentLeader(),focusS=lead.progress,t=smoothTangentAtS(focusS,135);
    const targetAngle=Math.atan2(t.y,t.x),targetX=lead.x-t.x*285,targetY=lead.y-t.y*285;
    camX+=(targetX-camX)*.032;camY+=(targetY-camY)*.032;
    let da=((targetAngle-camAngle+Math.PI*3)%(Math.PI*2))-Math.PI;
    da=clamp(da,-.016,.016);
    camAngle+=da;
    camS+=(focusS-camS)*.042;
  }
  ...
  function drawCourse(){
    ...
    const visible=path.filter(p=>p.s>camS-320&&p.s<camS+1320).map(p=>({p,pr:project(p.x,p.y)})).filter(o=>o.pr.visible);
    if(visible.length>2){
      const left=[],right=[];
      visible.forEach(o=>{
        const t=smoothTangentAtS(o.p.s,60),nx=-t.y,ny=t.x;
        left.push(project(o.p.x+nx*TRACK_HALF,o.p.y+ny*TRACK_HALF));
        right.push(project(o.p.x-nx*TRACK_HALF,o.p.y-ny*TRACK_HALF));
      });
      ...
      [left,right].forEach(edge=>{
        ...
        ctx.strokeStyle='#98a6bf';ctx.lineWidth=7;ctx.lineJoin='round';ctx.stroke();
        ctx.strokeStyle='#4c5970';ctx.lineWidth=3;ctx.stroke();
      });
    }
  ...
  function resetMarbles(){
    ...
    camLeaderId=0;camX=q.x-t.x*285;camY=q.y-t.y*285;camAngle=Math.atan2(t.y,t.x);camS=45;
