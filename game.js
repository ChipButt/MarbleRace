(() => {
  ...
  function buildCourse(){
    ...
    c.funnels.forEach((frac,i)=>{
      ...
      const scale=c.funnelSize||1, outerR=160*scale, holeR=30*scale;
      const side=i%2===0?1:-1;
      // Place the bowl off-centre so the chute crosses its outer half. This
      // makes incoming course velocity naturally tangential to the bowl.
      const offset=outerR*.62*side;
      funnels.push({
        id:i,s,
  ...
  function maybeEnterFunnel(m){
    ...
    for(const f of funnels){
      ...
      const prev=m.lastFunnelDistance.get(f.id);
      m.lastFunnelDistance.set(f.id,d);

      // Enter only when the real trajectory crosses the physical bowl rim.
      const captureR=f.outerR-R*.55;
      if(prev!==undefined && prev>captureR && d<=captureR){
        m.funnel={id:f.id,f};
        return;
      }
    }
  ...
  function updateFunnel(m,dt){
    ...
    // Deep parabolic bowl. Real entry velocity is preserved, allowing circular,
    // elliptical and collision-altered orbits while gravity pulls toward centre.
    const bowlOmega2=23.0;
    m.vx+=-dx*bowlOmega2*dt;
    m.vy+=-dy*bowlOmega2*dt;

    const damping=Math.exp(-.065*dt);
    ...
    // Once the marble physically overlaps the drain, it falls. No radial-speed
    // condition can leave it circling forever directly over the centre hole.
    if(d<f.holeR+R*.22){
  ...
  function updateCamera(){
    ...
    let weightTotal=0,focusX=0,focusY=0;
    for(const m of relevant){
      const gap=lead.progress-m.progress;
      const w=m.id===lead.id?3.2:Math.max(.45,1.5-gap/600);
      focusX+=m.x*w;focusY+=m.y*w;weightTotal+=w;
    }
    focusX/=weightTotal;focusY/=weightTotal;

    let targetX,targetY,targetAngle,targetHeight;
    if(lead.funnel){
      const f=lead.funnel.f;
      const t=smoothTangentAtS(f.s,145);
      targetAngle=Math.atan2(t.y,t.x);
      targetX=f.x-t.x*350;
      targetY=f.y-t.y*350;
      targetHeight=330;
    }else{
      const t=smoothTangentAtS(lead.progress,115);
      targetAngle=Math.atan2(t.y,t.x);
      const spread=Math.max(...relevant.map(m=>Math.hypot(m.x-focusX,m.y-focusY)),0);
      const back=295+clamp(spread*.35,0,110);
      targetX=focusX-t.x*back;
      targetY=focusY-t.y*back;
      targetHeight=245+clamp(spread*.28,0,105);
    }

    camX+=(targetX-camX)*.055;
    camY+=(targetY-camY)*.055;
    camHeight+=(targetHeight-camHeight)*.045;

    let da=((targetAngle-camAngle+Math.PI*3)%(Math.PI*2))-Math.PI;
    const maxTurn=Math.abs(da)>.7?.055:Math.abs(da)>.3?.038:.024;
    da=clamp(da,-maxTurn,maxTurn);
    camAngle+=da;
    camS+=(lead.progress-camS)*.06;
  }
  ...
  function resetMarbles(){
    ...
    marbles=COLORS.map((m,i)=>({
      ...
      usedFunnels:new Set(),funnel:null,lastFunnelDistance:new Map()
    }));
