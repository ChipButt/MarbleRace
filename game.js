(() => {
  ...
  function buildCourse(){
    ...
    c.funnels.forEach((frac,i)=>{
      ...
      const entryS=clamp(s-outerR*.96,80,totalLength-500);
      const exitS=clamp(s+outerR*.96,entryS+outerR*1.55,totalLength-100);
  ...
  function funnelLevelBefore(f){
    let level=0;
    for(const other of funnels){
      if(other.entryS<f.entryS) level+=other.depth;
    }
    return level;
  }

  function trackLevelAtS(s){
    let level=0;
    for(const f of funnels){
      if(s>=f.exitS) level+=f.depth;
    }
    return level;
  }
  ...
  function project3D(x,y,z=0){
    ...
    return{x:W*.5+lateral*scale,y:H*.18+(camHeight+z)*scale,scale,
      visible:forward>-140&&forward<1500,forward};
  }

  function project(x,y){
    return project3D(x,y,0);
  }
  ...
  function drawTrackSegment(s0,s1){
    ...
    const left=idx.map(i=>{
      const z=trackLevelAtS(path[i].s);
      return project3D(mesh.left[i].x,mesh.left[i].y,z);
    });
    const right=idx.map(i=>{
      const z=trackLevelAtS(path[i].s);
      return project3D(mesh.right[i].x,mesh.right[i].y,z);
    });

    ctx.beginPath();ctx.moveTo(left[0].x,left[0].y);left.slice(1).forEach(p=>ctx.lineTo(p.x,p.y));
    for(let i=right.length-1;i>=0;i--)ctx.lineTo(right[i].x,right[i].y);
    ctx.closePath();
    const g=ctx.createLinearGradient(0,H*.12,0,H);
    g.addColorStop(0,'#333d55');g.addColorStop(1,'#20283a');
  ...
  function projectedRingPoints(f,radius,z,steps=64){
    const pts=[];
  ...
  function drawLowerCatchTrack(f){
    const z=funnelLevelBefore(f)+f.depth;
    const end=pointAtS(f.exitS);
    const outT=tangentAtS(f.exitS,60);

    const start={x:f.x,y:f.y,z};
    const c1={x:f.x+outT.x*85,y:f.y+outT.y*85,z};
    const c2={x:end.x-outT.x*95,y:end.y-outT.y*95,z};

    const centre=[];
    for(let i=0;i<=24;i++){
      const u=i/24,o=1-u;
      centre.push({
        x:o*o*o*start.x+3*o*o*u*c1.x+3*o*u*u*c2.x+u*u*u*end.x,
        y:o*o*o*start.y+3*o*o*u*c1.y+3*o*u*u*c2.y+u*u*u*end.y,
        z
      });
    }
    ...
    for(let i=0;i<centre.length;i++){
      ...
      left.push(project3D(centre[i].x+nx*TRACK_HALF,centre[i].y+ny*TRACK_HALF,z));
      right.push(project3D(centre[i].x-nx*TRACK_HALF,centre[i].y-ny*TRACK_HALF,z));
    }
    ...
    for(let i=right.length-1;i>=0;i--)ctx.lineTo(right[i].x,right[i].y);
    ctx.closePath();
    ctx.fillStyle='#20283a';ctx.fill();

    for(const edge of [left,right]){
      ctx.beginPath();ctx.moveTo(edge[0].x,edge[0].y);
      for(let i=1;i<edge.length;i++)ctx.lineTo(edge[i].x,edge[i].y);
      ctx.strokeStyle='#aab6cb';ctx.lineWidth=7;ctx.lineJoin='round';ctx.lineCap='round';ctx.stroke();
      ctx.strokeStyle='#526078';ctx.lineWidth=3;ctx.stroke();
    }
  }

  function drawFunnel(f){
    const baseZ=funnelLevelBefore(f);
    const centre=project3D(f.x,f.y,baseZ+f.depth);
    if(!centre.visible)return;

    const ringCount=20,steps=64;
    const rings=[];
    ...
    for(let j=0;j<=ringCount;j++){
      ...
      rings.push(projectedRingPoints(f,radius,baseZ+z,steps));
    }

    for(let j=0;j<ringCount;j++){
      const outer=rings[j],inner=rings[j+1];
      const u=(j+.5)/ringCount;
      const light=Math.round(178-102*Math.pow(u,.82));
      const fill=`rgb(${light},${light+7},${Math.min(205,light+17)})`;
    ...
    const rim=projectedRingPoints(f,f.outerR,baseZ,steps);
    drawPolygon(rim,null,'#e4eaf2',Math.max(2,3*centre.scale));

    const hole=projectedRingPoints(f,f.holeR,baseZ+f.depth,steps);
  ...
  function drawFinish(){
    ...
    const z=trackLevelAtS(s);
    const a=project3D(q.x+nx*(TRACK_HALF-7),q.y+ny*(TRACK_HALF-7),z);
    const b=project3D(q.x-nx*(TRACK_HALF-7),q.y-ny*(TRACK_HALF-7),z);
    for(let i=0;i<12;i++){
  ...
  function drawCourse(){
    ...
    pegs.forEach(p=>{
      ...
      const q=project3D(p.x,p.y,trackLevelAtS(p.s));if(!q.visible)return;
  ...
  function drawMarble(m){
    let z=trackLevelAtS(m.progress);
    ...
    if(m.funnel){
      ...
      z=baseZ+f.depth*Math.pow(inward,1.55);
    }
